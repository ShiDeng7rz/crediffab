...
from diffab.modules.common.so3 import ApproxAngularDistribution, random_normal_so3, so3vec_to_rotation, \
    rotation_to_so3vec
...
class RotationTransition(nn.Module):
    ...
    def __init__(self, num_steps, var_sched_opt=None, angular_distrib_fwd_opt=None, angular_distrib_inv_opt=None):
        super().__init__()
        if var_sched_opt is None:
            var_sched_opt = {}
        if angular_distrib_fwd_opt is None:
            angular_distrib_fwd_opt = {}
        if angular_distrib_inv_opt is None:
    ...
    def add_noise(self, v_0, mask_generate, t):
        ...
        # Noise rotation
        e_scaled = random_normal_so3(t[:, None].expand(N, L), self.angular_distrib_fwd,
                                     device=self._dummy.device)  # (N, L, 3)
...
class AminoacidCategoricalTransition(nn.Module):
    ...
    def __init__(self, num_steps, num_classes=20, var_sched_opt=None):
        super().__init__()
        if var_sched_opt is None:
            var_sched_opt = {}
