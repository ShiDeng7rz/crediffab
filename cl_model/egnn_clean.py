...
class EGNN(nn.Module):
    def __init__(self, in_node_nf, hidden_nf, out_node_nf, in_edge_nf=0, device='cpu', act_fn=nn.SiLU(), n_layers=4,
        ...
        self.edge_out = nn.Sequential(
            nn.Linear(hidden_nf, hidden_nf),
            act_fn,
            nn.Linear(hidden_nf, in_edge_nf)
        )
        for i in range(0, n_layers):
            self.add_module("gcl_%d" % i,
                            E_GCL(self.hidden_nf, self.hidden_nf, self.hidden_nf, edges_in_d=self.hidden_nf,
...
def get_edges_batch(n_nodes, batch_size):
    ...
    edge_attr = torch.randn(len(edges[0]) * batch_size, 1)
